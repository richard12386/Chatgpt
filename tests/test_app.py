...
class RouteTests(unittest.TestCase):
    ...
    def test_backtest_get_page(self):
        response = self.client.get("/backtest")
        body = response.get_data(as_text=True)
        self.assertEqual(response.status_code, 200)
        self.assertIn("Strategy Backtest", body)

    @patch("app.get_backtest_result")
    def test_backtest_post_renders_result(self, backtest_mock: Mock):
        backtest_mock.return_value = app.BacktestResult(
            ...
            points=[
                ...
                {"date": "2026-02-01", "equity": 12345.0, "buy_hold": 11800.0},
            ],
        )
        response = self.client.post(
            "/backtest",
